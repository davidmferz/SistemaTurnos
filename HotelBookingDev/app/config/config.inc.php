<?php
if (!defined("PJ_HOST")) define("PJ_HOST", "localhost");
if (!defined("PJ_USER")) define("PJ_USER", "root");
if (!defined("PJ_PASS")) define("PJ_PASS", "");
if (!defined("PJ_DB")) define("PJ_DB", "yates");
if (!defined("PJ_PREFIX")) define("PJ_PREFIX", "");
if (!defined("PJ_INSTALL_FOLDER")) define("PJ_INSTALL_FOLDER", "/HotelBookingDev/");
if (!defined("PJ_INSTALL_PATH")) define("PJ_INSTALL_PATH", "C:/xampp/htdocs/HotelBookingDev/");
if (!defined("PJ_INSTALL_URL")) define("PJ_INSTALL_URL", "http://localhost/HotelBookingDev/");
if (!defined("PJ_SALT")) define("PJ_SALT", "88TH258U");
if (!defined("PJ_INSTALLATION")) define("PJ_INSTALLATION", "");
?>